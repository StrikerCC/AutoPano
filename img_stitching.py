...
def warp_img(img_src, img_tgt, homo_src_2_tgt):
    ...
    m, n = img_tgt.shape[:2]
    img_box_tgt = (0, 0, n, m)
    img_corners_tgt = slam_lib.geometry.xyxy_2_corners_coord(*img_box_tgt)

    m, n = img_src.shape[:2]
    img_box_src = (0, 0, n, m)
    img_corners_src = slam_lib.geometry.xyxy_2_corners_coord(*img_box_src)
    ...
    img_box_src_warp = slam_lib.geometry.corners_2_bounding_box_xyxy(img_corners_src_warped)

    img_box_tgt_plus_warped_src = slam_lib.geometry.merge_box([img_box_src_warp, img_box_tgt])
    ...
    img_box_tgt_in_out = slam_lib.geometry.corners_2_bounding_box_xyxy(img_corners_tgt_in_out)
    ...
    # compute overlapping
    img_blending, mask_overlapping = slam_lib.img_blending.blend_by_median(img_tgt, img_warped[int(img_box_tgt_in_out[1]):int(img_box_tgt_in_out[3]), int(img_box_tgt_in_out[0]):int(img_box_tgt_in_out[2])])
...
def stitch_with_last_img():
    data_path = './data/head'
    img_paths = slam_lib.dataset.get_iphone_data(data_path)
    print(img_paths)

    for i in range(len(img_paths)-1):
        img1, img2 = cv2.imread(img_paths[i]), cv2.imread(img_paths[i+1])
        homo_1_2_2 = homo.find_homo(img1, img2)
...
def stitch_with_first_img():
    data_path = './data/graf'
    img_paths, homo_paths = slam_lib.dataset.get_robots_ox_ac_uk_vgg_data(data_path)
    for i in range(len(homo_paths)):
        ...
        homo_1_2_2 = homo.find_homo(img1_org, img2)
...
def stitch_with_img_no_label():
    ...
    for i in range(len(img_paths)):
        ...
        img2 = cv2.imread(img2_path)
        homo_1_2_2 = homo.find_homo(img1_org, img2, flag_vis_feature_matching=True)
